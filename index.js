...
import { launchLoginBrowser, getFrame, getStatus, clickAt, typeText, pressKey, gotoUrl, autoFillLogin, killBrowser } from './lib/browserLogin.js';
...
// One Launch button → FRESH throwaway browser profile → aistudio. The admin
// completes whatever login the page shows. A watcher polls cookies and
// AUTO-SAVES the account the moment all three exist, then kills the browser.
// Kill guarantees: auto-save, 10-min hard cap, 3-min idle, Kill button, next
// Launch. No orphan chromium. No manual "check cookies" step.
app.post('/admin/browser/launch', adminAuth, async function (req, res) {
  try {
    var r = await launchLoginBrowser(getDB(), req.body.label || null, req.body.url || null);
    res.json(r);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.get('/admin/browser/frame', adminAuth, async function (req, res) {
  try { res.json(await getFrame()); }
  catch (e) { res.status(500).json({ error: e.message }); }
});

app.get('/admin/browser/status', adminAuth, function (req, res) {
  res.json(getStatus());
});

app.post('/admin/browser/kill', adminAuth, async function (req, res) {
  try { await killBrowser(); res.json({ status: 'idle', message: 'Browser killed.' }); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/admin/browser/fill', adminAuth, async function (req, res) {
  try {
    ...
    res.json({ message: 'credentials typed — review the frame and click Sign in', checkbox: r.checkbox });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/admin/browser/goto', adminAuth, async function (req, res) {
  try { await gotoUrl(req.body.url); res.json({ ok: true }); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
